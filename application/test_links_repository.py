...
# Фикстура для создания имитации подключения и курсора
@pytest.fixture
def mock_db():
    conn = MagicMock()
    cursor = MagicMock()
    # Настраиваем контекстный менеджер with conn.cursor() as cur:
...
def test_insert_data_success(mock_db):
    ...
    # Предположим, что DOMEN_FOR_SHORT_URL = "http://short.ly"
    ...
    assert cursor.execute.call_count == 2 # Должно быть 2 вызова: INSERT и UPDATE
...
def test_delete_link_for_id(mock_db):
    ...
    # Проверяем, что передан именно кортеж (5,)
